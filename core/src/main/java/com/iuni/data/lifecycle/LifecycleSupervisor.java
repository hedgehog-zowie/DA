...
public class LifecycleSupervisor implements LifecycleAware {
    ...
    public synchronized void supervise(LifecycleAware lifecycleAware, SupervisorPolicy policy, LifecycleState desiredState) {
        if (this.monitorService.isShutdown() || this.monitorService.isTerminated() || this.monitorService.isTerminating()) {
            throw new IuniDAException("Supervise called on " + lifecycleAware + " " +
            ...
            try {
                ...
                synchronized (lifecycleAware) {
                    if (supervisoree.status.discard) {
                        ...
                        logger.info("Component {} is in error state, and Flume will not attempt to change its state", lifecycleAware);
                        return;
                    }

                    supervisoree.status.lastSeenState = lifecycleAware.getLifecycleState();

                    if (!supervisoree.status.desiredState.equals(lifecycleAware.getLifecycleState())) {
                        ...
                        switch (supervisoree.status.desiredState) {
                            ...
                            case STOP:
                                try {
                                    ...
                                    logger.error("Unable to stop {} - Exception follows. {}", lifecycleAware, e);
                                    if (e instanceof Error) {
                                        throw (Error) e;
                                    }
                                    supervisoree.status.failures++;

                        ...
                        <div class="portlet-body">
                            <div class="table-toolbar">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="btn-group">
                                            <button id="table_buried_group_new" class="btn green">
                                                添加 <i class="fa fa-plus"></i>
                                            </button>
                                            <button id="table_buried_group_delete" class="btn red">
                                    ...
                                    <div class="col-md-6">
                                        <div class="btn-group pull-right">
                                            <button id="table_buried_group_export" class="btn yellow">
                            ...
                            <table id="table_buried_group" class="table table-striped table-bordered table-hover">
                                <thead>
                                <tr>
                                    <th class="table-checkbox"><input type="checkbox" class="group-checkable"
                                                                      data-set="#table_buried_group .checkboxes"/></th>
                                    <th>埋点组名称</th>
                                    <th>埋点个数</th>
                                    <th>添加时间</th>
                                    <th>备 注</th>
                                ...
                                <tbody>
                                <c:forEach items="${buriedGroupList}" var="buriedGroup">
                                    <tr class="odd gradeX">
                                        <td><input type="checkbox" class="checkboxes" value="${buriedGroup.id}"/></td>
                                        <td>${buriedGroup.name}</td>
                                        <td>${fn:length(buriedGroup.buriedRelations)}</td>
                                        <td>${buriedGroup.createDate}</td>
                                        <%--<td><fmt:formatDate value="${buriedGroup.createDate}" pattern="yyyy/MM/dd -- HH:mm:ss"/></td>--%>
                                        <td>${buriedGroup.desc}</td>
                                        <td>
                                            <a class="edit text-blue" id="edit_${buriedGroup.id}" href="javascript:;"><i
                                                    class="fa fa-pencil text-blue"></i> 编辑 </a>
                                            |
                                            <c:if test="${buriedGroup.status == 1}">
                                                <a class="disable text-yellow" id="disable_${buriedGroup.id}" href="javascript:;"><i
                                                        class="fa fa-stop text-yellow"></i> 禁用 </a>
                                            </c:if>
                                            <c:if test="${buriedGroup.status == 0}">
                                                <a class="enable text-green" id="enable_${buriedGroup.id}" href="javascript:;"><i
                                                        class="fa fa-play text-green"></i> 启用 </a>
                                            </c:if>
                                            |
                                            <a class="delete text-red" id="delete_${buriedGroup.id}" href="javascript:;"><i

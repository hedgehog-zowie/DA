 ...
 * 1. 国家记录 
 * 2. 地区记录 但是地区记录是不一定有的。而且国家记录和地区记录都有两种形式 1. 以0结束的字符串 2. 4个字节，一个字节可能为0x1或0x2 a.
 ...
 * 三. "起始地址/结束地址偏移"记录区 
 * 1. 每条记录7字节，按照起始地址从小到大排列 a. 起始IP地址，4字节 b. 结束ip地址的绝对偏移，3字节
 * 
...
public class IPSeeker {
	...
	private static final String IP_FILE = IPSeeker.class.getResource(
			"/com/iuni/data/iplib/iuni/iplibrary/purelibrary/qqwry.dat").getPath();
	...
	private IPSeeker() {
		...
		try {
			...
			System.out.println(IPSeeker.class.getResource("/com/iuni/data/iplib/iuni/iplibrary/purelibrary/qqwry.dat")
					.toString());
	...
	public List<IPEntry> getIPEntriesDebug(String s) {
		...
		for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
			...
			if (temp != -1) {
				...
				if (loc.area.indexOf(s) != -1 || loc.location.indexOf(s) != -1) {
		...
		try {
			...
			for (int offset = (int) ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
				...
				if (temp != -1) {
					IPLocation loc = getIPLocation(temp);
					// 判断是否这个地点里面包含了s子串，如果包含了，添加这个记录到List中，如果没有，继续
					if (loc.area.indexOf(s) != -1
							|| loc.location.indexOf(s) != -1) {
						IPEntry entry = new IPEntry();
						entry.area = loc.area;
						entry.location = loc.location;
						// 得到起始IP
	...
	public String getLocation(String ip) {
		return getLocation(Utils.getIpByteArrayFromString(ip));
	}
	...
	private IPLocation getIPLocation(int offset) {
		...
		if (b == AREA_FOLLOWED) {
			...
			b = mbb.get();
			if (b == NO_AREA) {
				loc.area = readString(readInt3());
				mbb.position(countryOffset + 4);
			} else
				loc.area = readString(countryOffset);
			// 读取地区标志
			loc.location = readArea(mbb.position());
		} else if (b == NO_AREA) {
			loc.area = readString(readInt3());
			loc.location = readArea(offset + 8);
		} else {
			loc.area = readString(mbb.position() - 1);
			loc.location = readArea(mbb.position());
		}
		return loc;
	}
	...
	public String getAddress(String ip) {
		String country = getArea(ip).equals(" CZ88.NET") ? ""
				: getArea(ip);
		String area = getLocation(ip).equals(" CZ88.NET") ? "" : getLocation(ip);

 ...
 *******************************/
$namespace("iuni.goods.normal");
/******************************
 　全局参数配置
 *******************************/
iuni.goods.normal.data={
	...
	haveGoods:false  //是否有商品售卖
};
iuni.goods.normal.dom={
...
/******************************
 　初始化配置
 *******************************/
iuni.goods.normal.init=function(){
	var that=this;
...
/******************************
 　功能实现区
 *******************************/
iuni.goods.normal.initData=function(){
	var that=this,
...
iuni.goods.normal.initAttr=function(){
	var that=this,
...
iuni.goods.normal.setAttribute=function(attrsSelectedArr){
...
iuni.goods.normal.getSKUProduct=function(selectedArr){
...
iuni.goods.normal.initQuantity=function(){
	...
	function setQuantity(num,maxValue,minValue) {
		...
		var max_buy_num=iuni.goods.normal.data.productData.max_buy_num,
			stock=iuni.goods.normal.data.productData.stock,
...
iuni.goods.normal.addListener=function(){
	var that=this;
	...
	var toCart=(function(){
		...
		return function (option,callback){
			...
			var num=option.num,
				...
				goods=[sku_id+":"+num];
			for(var i=0,len=option.service.length;i<len;i++){
				goods.push(option.service[i]+":"+num+":"+sku_id);
			}
			goods=goods.join(",");

			locked=true;
			$.ajax({
				url:"http://www.iuni.com/api/cart/add?dtype=json",
				type:"post",
				data:{
					"goods": goods
				},
	...
	function buy(elem){
		...
		toCart({num:num,sku_id:that.data.productData.sku_id,service:service},function(json){

			if(!json){
				$iuni_fail( '请求数据超时，请稍后重试');
				return ;
			}
			...
			//
			var errorCode=json.errorCode,msg=json.msg || "";

			if(errorCode===10 || errorCode===8){
				$iuni_customAlert({
					content : msg,
					submitBtnTxt : "确定"
				});
				...
				$iuni_customAlert({
					...
					submitBtnTxt : "立即支付",
					callback : function(){
						window.location = 'http://passport.iuni.com/myorder_detail.shtml?sn=' +json.data.ordersn;
					}
				});
...
iuni.goods.normal.changeBuyStatus=function(status,hint){
	var that=this,
		dom=that.dom;
	...
	var product=that.data.productData;
	//dom.chooseResult.innerHTML=product.goods_name+" "+attrName.join("  ");

	var dis="acc_buyBtn  acc_buyBtn_false",can="acc_buyBtn",nostock="acc_buyBtn   acc_buyBtn_oos";
	dom.btn_buy.setAttribute("no_goods",0);
	//dom.btn_buy2.style.display="none";
	that.data.haveGoods=false;

	if(status){

		dom.goodsPrice.style.display="";
		dom_service.show();

		if(product.is_sale){

			//dom.btn_buy2.style.display="";
			that.data.haveGoods=true;

			if(product.stock>0){
				dom.btn_buy.innerHTML='<span>加入购物车</span>';
				dom.btn_buy.className=can;
				dom.btn_buy2.innerHTML='加入购物车';
			}else{
				dom.btn_buy.innerHTML='<span>缺货登记</span>';
				dom.btn_buy.className=nostock;
				dom.btn_buy2.innerHTML='缺货登记';
			}
		...
		dom.goodsPrice.style.display="none";
		dom.btn_buy.className=dis;
		dom.btn_buy.innerHTML='<span>'+(hint || "无此商品")+'</span>';
		dom_service.hide();

	}
}
iuni.goods.normal.initImage=function(){
...
iuni.goods.normal.initReco=function(){
	var that=this;
...
iuni.goods.normal.initTab=function(){
	...
	function navFloat(){
		...
		that.dom.btn_buy2.style.display="none";
		...
		if(srch<fixH){
			...
			if(that.data.haveGoods){
				that.dom.btn_buy2.style.display="";
			}
		}
	}

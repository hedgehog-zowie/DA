 ...
 *******************************/
$namespace("iuni.template.firm_order");
/******************************
 　全局参数配置
 *******************************/

iuni.template.firm_order.data = {};
/******************************
 　初始化配置
 *******************************/

iuni.template.firm_order.set_default_value = function( dom, value ){
...
iuni.template.firm_order.get_checked = function( nodes ){	//取得选中的radio
	var node;
	$.each( nodes, function(){
		if ( $(this).is( ':checked' ) ){
			return node = $( this );
...
iuni.template.firm_order.Address = function(){


	var layer = function(){	//增加收货地址浮层


		var bindRegionSelect = function( ary, callback ){	//绑定省市县选择框事件
		...
		var render = iuni.app.getSingle( function( __parent, address_list, callback ){
			...
			var address_value = iuni.template.firm_order.set_default_value( address, '请填写具体地址，不需要重复填写省/市/区' );

			var tel_value = iuni.template.firm_order.set_default_value( tel, '区号-电话号码-分机号(可选)' );
		...
		var show = function( __parent, address_list, callback, mode ){
			...
			iuni.template.firm_order.set_default_value( doms.address, '请填写具体地址，不需要重复填写省/市/区' );
			iuni.template.firm_order.set_default_value( doms.tel, '区号-电话号码-分机号(可选)' );
...
iuni.template.firm_order.Payment = function(){
	...
	var check_payment_type = function( payment_type, payment_1, shipping_list, order_sn ){
		...
		change_shipping_list.done( function( data ){
			...
			setTimeout(function(){
				iuni.template.firm_order.compute_price();
			}, 100);
		...
		payment_type.on( 'click', function(){		//选择在线支付或者货到付款
			...
			if ( this.value === '1' ){	//在线支付
				param.pay_type = 2;
				iuni.template.firm_order.Payment.get_tab().show();
			}else{	//货到付款
				param.pay_type = 1;
				iuni.template.firm_order.Payment.get_tab().hide();
			}
	...
	var get_payment_type = function( payment_type ){		//查询已选择的支付方式(在线支付或者货到付款)
		return iuni.template.firm_order.get_checked( payment_type ).val();
	};
	...
	var get_checked = function(){
		return iuni.template.firm_order.get_checked( tab.getCurrTab().target.find( 'input' ) );
	};
...
iuni.template.firm_order.compute_price = function( shipping_list, use_conpounNumber, coupon_list,user_conponNumber_wrap, order_sn ){		//切换配送方式或者现金券之后重新计算商品价格
	...
	change_request.done( function( data ){
		...
		iuni.template.firm_order.data.order_amount = data.order_amount;
	});
	...
	var shipping_list_radio_val = iuni.template.firm_order.get_checked( shipping_list_radio ).val();

	var coupon_list_radio_val = iuni.template.firm_order.get_checked( coupon_list_radio ).val()||"";

	var conpousInput = $('[data-action="conpousText"]'),conpousText = conpousInput.val();

	var valida = iuni.template.firm_order.validate_conpounNumber( conpousInput, conpousText);
	...
	function ordioschange(ev){
		...
		coupon_list_radio_val = iuni.template.firm_order.get_checked( coupon_list_radio ).val()||"";

		var shipping_list_radio = shipping_list.find( 'input' );

		shipping_list_radio_val = iuni.template.firm_order.get_checked( shipping_list_radio ).val();
		...
		change_request.setParam({
			...
			coupon_sn: iuni.template.firm_order.coupon_type !== 2 ? ( coupon_list_radio_val || '' ) : ''
		});
	...
	use_conpounNumber.on('click',function(ev){   // 激活优惠码事件
	...
	iuni.template.firm_order.compute_price = function(){		//初始化之后改写自身，共用之前的变量

		return function( _type ){

			iuni.template.firm_order.coupon_type = _type || iuni.template.firm_order.coupon_type ;
			...
			if ( iuni.template.firm_order.coupon_type !== 2 ){		//type!==1为不使用现金券，传coupon_sn到后台
...
iuni.template.firm_order.coupon_type = null;

iuni.template.firm_order.user_conpon_list = function( user_conpon_list, coupon_list ){		//使用现金券
	...
	user_conpon_list.on( 'click', function(){
		...
		if ( use_icon.hasClass( 'use_icon' ) ){
			...
			text.html ( '取消使用' );
			iuni.template.firm_order.compute_price( 1 );
		}else{
			use_icon.removeClass( 'use_icon2' ).addClass( 'use_icon' );
			text.html ( '使用优惠券' );
			//$( '[data-action="coupon_radio"]:checked' ).removeAttr("checked");
			iuni.template.firm_order.compute_price( 2 );
		}
...
iuni.template.firm_order.validate_conpounNumber = function( dom,val ){		//验证优惠码
...
iuni.template.firm_order.submit = function( submit_btn, payment_type, payment_1, order_sn, add_address_btn, need_inv, inv_value, inv_change ){
	...
	inv_change.on( 'change', function(){
		...
		var val = this.value,
			...
			address_checked = iuni.template.firm_order.get_checked( address_radio ),
	...
	submit_btn.on( 'click', function(){
		...
		setTimeout(function(){
			...
			var address_radio = $( '[data-action="address_radio"]' ),
				firm_order = iuni.template.firm_order;
			...
			var pay_type = firm_order.get_checked( payment_type ).val(),
				pay_code,
				address_id,
				ship_code,
				coupon_sn,
				...
				inv_content = '',
			//订单金额,需传回后台做校验
				order_amount = firm_order.data.order_amount || $("#hdnOrderAmount").val();


			if ( pay_type === '1' ){	//在线支付
				pay_code = firm_order.Payment.get_checked().attr( 'data-pay_code' );
			}else{
				pay_code = 'cod';	//货到付款
			}

			address_id = firm_order.get_checked( address_radio ).val();

			ship_code = firm_order.get_checked( shipping_list_radio ).val();


			//优惠券
			coupon_sn = iuni.template.firm_order.coupon_type !== 2 ? ( firm_order.get_checked( coupon_radio ).val() || '' ) : '';
			...
			if ( need_inv.is( ':checked' ) ){
				...
				inv_title = inv_change.val() + inv_title;
			...
			var submit_request = $Request.create({
				...
				type: 'iframe',
				lock: true,
				target: pay_type === '0' ? 'self': ( order_amount - 0 === 0 ? 'self' : 'blank' )
			});
			...
			window.pay_callback = function( order_sn, order_url ){
				iuni.template.firm_order.show_aftersubmit_layer( order_sn, order_url );
			};

			/*//在线支付（金额大于0），提交失败提示
			 window.pay_callback_fail = function(){
			 iuni.common.ui.fail('提交失败');


			 };*/

			if ( isNotice ){
				return $iuni_customAlert({
					title: '提示', //非必填,
					content: '非常抱歉，'+ isNotice +'不足', //必填',
					submitBtnTxt: '继续下单',
					customBtnTxt: '返回购物车修改',
					callback: function() {
						submit_request.start();
					}, //必填
					customCallback: function() {
						return location.href = 'http://www.iuni.com/cart'
					}
				});
			}
...
iuni.template.firm_order.show_aftersubmit_layer = function( order_sn ){
...
iuni.template.firm_order.deposit = function( btnDeposit , conDeposit ){
...
iuni.template.firm_order.inv = function( need_inv, inv_info ){
...
iuni.template.firm_order.initPackage=function(){
	$("[action_sel=package_icon]").click(function(e){
		var css="isShow",
			othis=$(this);

		if(othis.hasClass(css)){
			othis.removeClass(css).html('<i class="iunifont">&#243;</i><span>套装内含商品</span>');
			othis.parent().parent().nextAll("[action_sel=packages]").hide();
		}else{
			othis.addClass(css).html('<i class="iunifont">&#244;</i><span>套装内含商品</span>');
			othis.parent().parent().nextAll("[action_sel=packages]").show();
		}
		e.preventDefault();
	});
};

iuni.template.firm_order.init = function(){
	...
	firm_order.Address.render_address_list( address_list, add_address_btn );	//收货地址列表

	firm_order.Address.add_address( add_address_btn, add_address_layer, address_list );	//增加收货地址

	firm_order.Payment.payment_mode( payment_1 );	//支付方式列表

	firm_order.Payment.check_payment_type( payment_type, payment_1, shipping_list, order_sn );	//选择在线支付或者货到付款


	firm_order.compute_price( shipping_list, use_conpounNumber, coupon_list,user_conponNumber_wrap, order_sn );	//配送方式和优惠券改变后重新计算

	firm_order.user_conpon_list( user_conpon_list, coupon_list );	//切换是否使用优惠券

	firm_order.inv( need_inv, inv_info );	//切换是否需要发票

	firm_order.deposit( btnDeposit , conDeposit );

	inv_info.focus();

	firm_order.submit( submit_btn, payment_type, payment_1, order_sn, add_address_btn, need_inv, inv_value, inv_change)	//提交订单

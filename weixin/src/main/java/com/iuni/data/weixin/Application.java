...
public class Application {
    ...
    public void start(CommandLine commandLine) throws ApiException {
        ...
        if (manual) {
            ...
            Date startDate = DateUtils.simpleDateStrToDate(startDateStr, "yyyyMMdd");
            Date endDate = DateUtils.computeStartDate(DateUtils.simpleDateStrToDate(endDateStr, "yyyyMMdd"), 1);
    ...
    public static void main(String[] args) throws ApiException, IOException {
        ...
        try {
            ...
            option = new Option("s", "start", true, "手动模式 - 开始时间（包括），开始时间需小于结束时间，格式为：yyyyMMdd");
            options.addOption(option);

            option = new Option("e", "end", true, "手动模式 - 结束时间(不包括)，结束时间需大于开始时间，格式为：yyyyMMdd");
            options.addOption(option);
            ...
            try {
                ...
                new HelpFormatter().printHelp("fetch weixin pay data", options, true);
                return;
            }
